...
export default function BunnyCharacter({
  ...
  const ROAM_SCALE = { normal: 0.4, hovered: 0.5 };
  useFrame(({ clock }) => {
    if (meshRef.current) {
  ...
  return (
    <group>
      ...
      <Html center position={[0, 2.2, 0]} style={{ pointerEvents: 'none' }}>
        <div style={{ color: '#fff', fontWeight: 'bold', textShadow: '0 1px 4px #000', fontSize: 18 }}>Bunny</div>
      </Html>
      ...
      {showMessage && (
        <Html center position={[0, 3.2, 0]} style={{ pointerEvents: 'none' }}>
          <div style={{
            ...
            Hi, I'm Bunny! Let's hop around and explore together!
          </div>
        </Html>
      )}
    </group>
