...
interface KizunaAnimeGirlProps {
  initialPosition?: [number, number, number];
  roamRadius?: number;
  onInteract?: (pos?: [number, number, number]) => void;
}

export default function KizunaAnimeGirl({
  initialPosition = [0, 1, 6],
  roamRadius = 4,
  onInteract,
}: KizunaAnimeGirlProps) {
  ...
  // Load Kizuna's GLB model
  const gltf = useGLTF('/Kizuna.glb');
  ...
  return (
    <group>
      ...
      <Html center position={[0, 2.2, 0]} style={{ pointerEvents: 'none' }}>
        <div style={{ color: '#fff', fontWeight: 'bold', textShadow: '0 1px 4px #000', fontSize: 18 }}>Kizuna</div>
      </Html>
      ...
      {showMessage && (
        <Html center position={[0, 3.2, 0]} style={{ pointerEvents: 'none' }}>
          <div style={{
            ...
            Konnichiwa! I'm Kizuna! Let's explore the anime universe together!
          </div>
        </Html>
      )}
    </group>
